...
#   select(id, listing_url, price, guests_included, cleaning_fee, extra_people, prop_type_simplified, number_of_reviews, review_scores_ratings, room_type, bathrooms, bedrooms, beds, accommodates, host_is_superhost, is_location_exact, neighbourhood_cleansed, cancellation_policy)
```
...
# calculate maximum (most often?) occurring price for 4 nights 
...
ggplot(listings_new,aes(x=price_4_nights))+
  geom_density() +
  geom_vline(xintercept = density(listings_new$price_4_nights)$x[which.max(density(listings_new$price_4_nights)$y)]) # plot vertical line to show maximum value
...
for every increase in rating by 1 point, the price goes up by 0.0023$ ??? -> check with log transformation
...
The number of bathrooms, bedrooms, beds, and size of the house (accommodates) are all significant predictors of price_4_nights. However, there might be a problem with multi-collinearity since intuitively bathrooms, bedrooms and beds should have a positive relationship with size of the house. A further analysis on VIF shows that size of the house has slight correlation with other predictors but is within an acceptable range.
# beds is also not signifiant? -> check this
...
```
Since the p-value for the estimate of the superhost coefficient is  significant (with a p-value of 0.36 and a t-statistic of 0.9), it seems that superhosts do command a price premium from their guests in Copenhagen. -> interpret coefficient 
...
autoplot(model4b)
```
Interaction variable is not significant, do not include.
...
```
A listing’s exact location is not a significant predictor of price_4_nights, thus we will not include it in the following models 
...
```
> One can see that the neighbourhood of an apartment is an predictor of the price for 4 nights in Copenhagen with a t-stat of around 16 and a resulting very low p-value. 
The highest price is by far in the Center, followed by Brokvarterer, which makes sense because it is a ring around the city center. Amager has prices in the middle range, while prices in the West and Nordvest are the lowest. These are more residential areas, farer away from the center, which explains the lower price.

#### Model 6 adjusted R2: 50%
...
Now, we are planning to visit Copenhagen and want to stay in an AirBnB. We would like to look at AirBnBs that are apartments with a private room, have at least 10 reviews, and an average rating of at least 90. 
...
# there are no values for Townhouse or Other after filtering for our conditions, so we exclude the from the dataset
...
# facet by prop_type_simplified -> still change order for proptype
ggplot(model_predictions, aes(x = 10^(.fitted))) +
  geom_density() +
  labs(y = "Predicted leave share",
       x = "Proportion of constituency with a university degree") +
...
# facet by neighbourhood & prop_type_simplified -> still change order for proptype
ggplot(model_predictions, aes(x = 10^(.fitted))) +
  geom_density() +
  labs(y = "Predicted leave share",
       x = "Proportion of constituency with a university degree") +
...
# predict for differnt nghds cuy they are signpred
predicted_value%>%group_by(neighbourhood)%>%summarise(mean=mean(price),mean_ratings=mean(review_scores_value))

```

```{r}
#Train-test splitting of dataset to check model accuracy
library(caret)
partition <- createDataPartition(y =listings_log$price_4_nights_log, p = 0.8, list = F)
trainingdata = listings_log[partition, ]
test <- listings_log[-partition, ]
```

```{r}
library(kableExtra)
p <- predict(final_model, trainingdata)
error <- (p- trainingdata$price_4_nights_log)
RMSE_Model <- sqrt(mean(error^2,na.rm=TRUE))

RMSE_Model

ptest <- predict(final_model, test)
error1 <- (ptest- test$price_4_nights_log)
RMSE_NewData <- sqrt(mean(error1^2,na.rm=TRUE))
...
Sources: https://en.wikipedia.org/wiki/Districts_of_Copenhagen
