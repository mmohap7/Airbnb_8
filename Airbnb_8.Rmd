...
```{r}
# already included above, can be deleted
...
```
Since the p-value for the estimate of the superhost coefficient is not significant, it seems that superhosts do not command a price premium from their guests in Copenhagen. 

Model 4: 40% -> decrease, do not include superhost in next models 
...
Additionally, property_type became significant again in this model, so it will be included again in the following models. 
...
We are planning to visit Copenhagen, and  want to stay in an Airbnb. We would like to look at Airbnbs that are apartment with a private room, have at least 10 reviews, and an average rating of at least 90. 

We will predict the total cost to stay at this Airbnb for 4 nights, include the appropriate 95% interval with the prediction. (Report the point prediction and interval in terms of price_4_nights)
