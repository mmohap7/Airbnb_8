...
After running regression models with these variables we came to a model that had the most prediction power, model 7. With the specified criteria we made a prediction with 95% confidence that the cost for a median listing on Airbnb in Copenhagen is between 1,860.18 USD and 1,938.58 USD.
...
```{r summarising minimum nights, echo=FALSE}
...
# locate the median value property and since there is an even number of observations, there are two median properties, which is why we add a small margin of error (0.1) to locate these properties 
predicted_value <- model_predictions %>%
  filter(fitted_price_final <= median(fitted_price_final, na.rm = TRUE)+0.1, 
         fitted_price_final >= median(fitted_price_final, na.rm = TRUE)-0.1)
...
>>>>>>> 79403f8c9bb894d2e7edb776676b073e8dbf371f
>>>>>>> 827be2a759c9eff123198967d647c7155056b96f
...
# Conclusion
To conclude, we can predict, with 95% confidence, that a median listing on AirBnB in Copenhagen, for 2 people and 4 nights, will cost between 1,860.18 USD and 1,938.58 USD.
